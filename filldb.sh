...
tail -n +2 "$(pwd)/csv/users.csv" | while IFS=";" read -r nom prenom fonction;
do
...
## csv pc 
sleep 5
tail -n +2 "$(pwd)/csv/pc.csv" | while IFS=";" read -r marque modele os ram date_achat id_user;
do
    # Requête SQL d'insertion pour chaque ligne du CSV
...
## csv logiciel 
sleep 5
tail -n +2 "$(pwd)/csv/logiciel.csv" | while IFS=";" read -r nom version id_pc;
do
    # Requête SQL d'insertion pour chaque ligne du CSV
...
## csv technicien 
sleep 5
tail -n +2 "$(pwd)/csv/technicien.csv" | while IFS=";" read -r nom prenom;
do
    # Requête SQL d'insertion pour chaque ligne du CSV
...
## csv maintenance 
sleep 5
tail -n +2 "$(pwd)/csv/maintenance.csv" | while IFS=";" read -r type description date_mt id_pc id_technicien;
do
    # Requête SQL d'insertion pour chaque ligne du CSV
